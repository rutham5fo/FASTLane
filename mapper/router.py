...
class router:
    ...
    def __init__ (self, mapper_context=None, cgra_context=None, logger_name: str='', log_level: int=logging.INFO, log_dir: str='logs') -> None:
        ...
        self.max_resolve_conflict_recursion = 100
    ...
    def assert_path_tracker_safe (self, block) -> bool:
        fn_name = router.assert_path_tracker_safe.__name__
        # Make sure all destinations belong to the block
        # Make sure all destinations are unique within the blk_path_tracker list
        tracker_safe = True
        blk_path_tracker = self.mapper_ctxt.data_path_tracker[block]
        port_cnt = len(blk_path_tracker)
        for port in range(port_cnt):
            path_cnt = len(blk_path_tracker[port])
            rel_path_tracker = [int(dest%self.cgra_ctxt.cgra_block_size) for dest in blk_path_tracker[port]]
            for i in range(path_cnt):
                dest_cnt = 0
                dest = blk_path_tracker[port][i]
                for comp in rel_path_tracker:
                    if (dest == comp):
                        dest_cnt += 1
                        if (dest_cnt == 2):
                            self.logger.error(f'{fn_name} ||| Found repeated destination[{dest}] in block[{block}], port[{port}]\'s tracker \n blk_path_tracker[{port}] = {blk_path_tracker[port]} | rel_path_tracker[{port}] = {rel_path_tracker}')
                            tracker_safe = False
                            break
                if (not tracker_safe):
                    break
    ...
    def resolve_conflict (self, path_type: int, conflict_pe: int, conflict_stack: list[int], recursion_cnt: int=0) -> bool:
        ...
        resolved = False
        # Sanity check
        if (recursion_cnt > self.max_resolve_conflict_recursion):
        ...
        n_port_meta = self.mapper_ctxt.pe_meta[conflict_pe]['data_port_meta'] if (path_type == 0) else self.mapper_ctxt.pe_meta[conflict_pe]['pred_port_meta']
        # Rotate metadata
        self.logger.debug(f'{fn_name} ||| Rotating ports of PE[{conflict_pe}] | n_out_opID = {n_out_opID}, n_out_blockID = {n_out_blockID}, n_port_children = {n_port_meta}')
        port_cnt = len(n_out_opID)
        t_out_opID, t_out_blockID, t_port_children = n_out_opID[0], n_out_blockID[0], n_port_meta[0]
        for i in range(port_cnt):
            # Delete current child_pe from tracker
            ch_pe = n_port_meta[i][0]
            if (ch_pe is not None):
                block = n_out_blockID[i]
                cblk_path_tracker = self.mapper_ctxt.data_path_tracker[block] if (path_type == 0) else self.mapper_ctxt.pred_path_tracker[block]
                cblk_src_tracker = self.mapper_ctxt.data_source_tracker[block] if (path_type == 0) else self.mapper_ctxt.pred_source_tracker[block]
                self.logger.debug(f'{fn_name} ||| Before removal of ch_pe[{ch_pe}] from port[{i}]: \n blk_path_tracker[{block}] = {cblk_path_tracker} \n blk_src_tracker[{block}] = {cblk_src_tracker}')
                self.del_tracker(cblk_path_tracker, cblk_src_tracker, i, conflict_pe)
            # Get new values
            if (i != port_cnt - 1):
                n_out_opID[i], n_out_blockID[i], n_port_meta[i] = n_out_opID[i+1], n_out_blockID[i+1], n_port_meta[i+1]
            else:
                n_out_opID[i], n_out_blockID[i], n_port_meta[i] = t_out_opID, t_out_blockID, t_port_children
            ch_pe = n_port_meta[i][0]
            ...
            if (ch_pe is not None):
                ...
                # Reflect rotation in child
                ch_in_opID[i] = n_out_opID[i]
        self.logger.debug(f'{fn_name} ||| Post rotation of PE[{conflict_pe}] | n_out_opID = {n_out_opID}, n_out_blockID = {n_out_blockID}, n_port_children = {n_port_meta}')
    ...
    def find_valid_port (self, path_type: int, block: int, n_pe: int, ch_pe: int, n_opID: int, n_out_opID: list, n_out_blockID: list, 
                         blk_path_tracker: list, blk_src_tracker: list, port_meta: list, attempted_ports: list, recursion_cnt: int=0) -> bool:
        ...
        # Get block child_pe since routing destination is relative to the current block
        min_dest = 0 if (block == self.cgra_ctxt.cgra_blocks-1) else (block+1)*self.cgra_ctxt.cgra_block_size
        blk_ch_pe = rel_ch_pe + min_dest
        if (self.mapper_ctxt.pe_meta.get(blk_ch_pe, None) is None):
            ...
            self.mapper_ctxt.create_pe_meta(blk_ch_pe)
        ch_in_opID = self.mapper_ctxt.pe_meta[blk_ch_pe]['data_in_opID'] if (path_type == 0) else self.mapper_ctxt.pe_meta[blk_ch_pe]['pred_in_opID']
        for i in range(len(n_out_opID)):
            self.logger.debug(f'{fn_name} ||| Iteration[{recursion_cnt}] | n_pe = {n_pe}, n_opID = {n_opID}, ch_pe = {ch_pe}, blk_ch_pe = {blk_ch_pe} | n_out_opID = {n_out_opID}, n_out_blockID = {n_out_blockID}, ch_in_opID = {ch_in_opID}, attempted_ports = {attempted_ports} | blk_path_tracker = {blk_path_tracker}')
            ...
            port_attempted = i in attempted_ports
            self.logger.debug(f'{fn_name} ||| port_sel[{i}] | rel_ch_pe = {rel_ch_pe}, rel_blk_path_tracker[{block}] = {rel_blk_path_tracker}, ports_attempted = {attempted_ports} | ch_exists_inPath = {ch_exists_inPath}, port_attempted = {port_attempted}')
            if (not ch_exists_inPath and not port_attempted):
                ...
                if (n_out_opID[i] is None):
                    ...
                    port_meta[i][0] = blk_ch_pe
                    # Add to attempted ports
                    attempted_ports.append(i)
                    port_meta[i][1] = attempted_ports
                    # Add path in block's path&source-trackers
                    self.add_tracker(blk_path_tracker, blk_src_tracker, i, n_pe, blk_ch_pe)
                    routed = True
                    break
                # Else, mark it as a possible port to place in and continue to find
                # a better solution, i.e., a port that is None/free.
                else:
                    self.logger.debug(f'{fn_name} ||| Marking port[{i}], n_out_opID = {n_out_opID} as possible')
        ...
        if (not routed and len(possible_ports) > 0):
            ...
            prt = [pp for pp in possible_ports if (not pp in attempted_ports)][0]
            aux_n_opID = n_out_opID[prt]
            aux_n_blockID = n_out_blockID[prt]
            aux_ch_pe = port_meta[prt][0]
            aux_attempted_ports = port_meta[prt][1]
            ...
            self.logger.debug(f'{fn_name} ||| prt = {prt}, aux_n_opID = {aux_n_opID}, aux_n_blockID = {aux_n_blockID}, aux_ch_pe = {aux_ch_pe} | blk_path_tracker[{block}][{prt}] = {blk_path_tracker[prt]}')
            self.del_tracker(blk_path_tracker, blk_src_tracker, prt, n_pe)
            self.add_tracker(blk_path_tracker, blk_src_tracker, prt, n_pe, blk_ch_pe)
            attempted_ports.append(prt)
            port_meta[prt][0] = blk_ch_pe
            port_meta[prt][1] = attempted_ports
            self.logger.debug(f'{fn_name} ||| prt = {prt}, aux_n_opID = {aux_n_opID}, aux_n_blockID = {aux_n_blockID}, aux_ch_pe = {aux_ch_pe} | blk_path_tracker[{block}][{prt}] = {blk_path_tracker[prt]}')
            # Recursive call with aux_n_opID and aux_ch_pe to place displaced edge
            recursion_cnt += 1
            routed = self.find_valid_port(path_type, aux_n_blockID, n_pe, aux_ch_pe, aux_n_opID, n_out_opID, n_out_blockID, 
                                          blk_path_tracker, blk_src_tracker, port_meta, aux_attempted_ports, recursion_cnt)
        if (not routed):
            ...
            if (ploc is None):
                ...
                # Find conflicting PE
                conflict_pe, blk_tracker_loc = self.find_conflicting_pe(blk_path_tracker, blk_src_tracker, ploc, n_pe, blk_ch_pe)
                if (conflict_pe is not None):
                    # Update tracking info
                    port_meta[ploc][0] = blk_ch_pe
                    self.logger.debug(f'{fn_name} ||| prt = {ploc}, n_opID = {n_opID}, n_block = {block}, ch_pe = {ch_pe}, blk_ch_pe = {blk_ch_pe} | n_out_opID = {n_out_opID}, n_out_blockID = {n_out_blockID} | conflicting_pe = {conflict_pe} @ {blk_tracker_loc} \n blk_path_tracker[{block}] = {blk_path_tracker}, blk_src_tracker[{block}] = {blk_src_tracker}')
                    self.add_tracker(blk_path_tracker, blk_src_tracker, ploc, n_pe, blk_ch_pe)
                    # Call resolver
                    self.logger.debug(f'{fn_name} ||| Attempting to resolve conflict start @ PE[{conflict_pe}] \n blk_path_tracker[{block}] = {blk_path_tracker} \n rel_blk_path_tracker[{block}] = {[[int(t%self.cgra_ctxt.cgra_block_size) for t in blk_path_tracker[i]] for i in range(self.cgra_ctxt.cgra_radix)]} \n blk_src_tracker[{block}] = {blk_src_tracker}')
    ...
    def route_node_path (self, dedges, n_name, n_opID, n_pe, n_shadow_pe, n_children, n_blk, n_shadow_blk, data_path_tracker, data_source_tracker, pred_path_tracker, pred_source_tracker) -> bool:
        ...
        for cid, ch in enumerate(n_children):
            ...
            if (path_type is not None):
                ...
                # Reset port_meta
                self.mapper_ctxt.reset_port_meta(n_pe_sel)
                # Get node metadata
                ...
                port_meta = self.mapper_ctxt.pe_meta[n_pe_sel]['data_port_meta'] if (path_type == 0) else self.mapper_ctxt.pe_meta[n_pe_sel]['pred_port_meta']
                ...
                self.logger.debug(f'{fn_name} ||| Routing Edge[{cid}] from src_PE[{n_pe}] -> dest_PE[{ch_pe}], in block[{n_blk_sel}]\'s path')
                # Find a port that does not contain the destination PE in its path-set
                routed = self.find_valid_port(path_type, n_blk_sel, n_pe_sel, ch_pe, n_opID, n_out_opID, n_out_blockID, 
                                              blk_path_tracker, blk_src_tracker, port_meta, attempted_ports)
                self.logger.debug(f'{fn_name} ||| Routed = {routed} | Result: src_PE[{n_pe}], Routing_PE = {n_pe_sel}, src_Node[{n_name}], out_opID = {n_out_opID}, out_blockID = {n_out_blockID}; dest_PE[{ch_pe}], dest_Node[{ch_name}] \n blk_path_tracker[{n_blk_sel}] = {blk_path_tracker} \n blk_src_tracker[{n_blk_sel}] = {blk_src_tracker}')
                if (not routed):
                    ...
                    # Block path tracker Sanity check
                    if (not self.assert_path_tracker_safe(n_blk_sel)):
                        routed = False
        return routed
