...
- recall_step_weight = 2
- **initial_recall_step (DERIVED, NOT constant)** = `ceil(block_size / (len(allowed_pes) * recall_step_weight))`
...
- Each DFG iteration must start with the initial condition:
  ...
  - `fixed_PE`, `rep_PE` and `vic_PE` are **shared** by all blocks. This is to ensure all blocks are symmetric.
  - `fixed_PE` list is empty.
  - `rep_PE`, `vic_PE` are both None.
  - `recall_step` = `ceil(block_size / (len(allowed_pes) * recall_step_weight))` (recomputed when block_size changes)
- A transfer will re-assign the number of allowed PEs of one type to another, uniformly across all blocks.
  - Ex: If `rep_PE` is `PE_int` and `vic_PE` is `PE_float`, a transfer of `recall_step` will reduce the number of PEs of type `vic_PE` by `recall_step` in EACH block and increment `rep_PE` by the same value in EACH block.
  - **CRITICAL:** The same `recall_step` amount is transferred in each block independently. After transfer, all blocks have identical composition (symmetric).
...
RECALL_STEP_WEIGHT = <recall_step_weight>

def compute_initial_recall_step(block_size):
    import math
    return math.ceil(block_size / (len(ALLOWED_PES) * RECALL_STEP_WEIGHT))
                ...
                if "Mapping: SUCCESS" in output:
                    ...
                    if "Opcode[" in output:
                        ...
                        if rep_PE and vic_PE and not pe_has_available_count(composition, vic_PE):
                            ...
                            if new_vic_PE:
                                vic_PE = new_vic_PE
                                # Transfer: reduce vic_PE by recall_step in EACH block, increment rep_PE by recall_step in EACH block
...
- [ ] For each block count (2-8):
  ...
  - [ ] For each matching DFG:
    - [ ] Initialize: current_block_size = initial_block_size (8), compute recall_step = ceil(block_size / (len(allowed_pes) * recall_step_weight)), fixed_PE = [], rep_PE = None, vic_PE = None
