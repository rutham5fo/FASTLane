...
cmd = Command list
max_blocks = 8
result_dir = varBlks_u4b2d2
for (blk in range(2, max_blocks+1)):
    set 'blocks' in cgra_config.yaml = blk
    if (blk == 2):
        set 'overload' in cgra_config.yaml = 0
    else:
        set 'overload' in cgra_config.yaml = 1
    for (dfg_name in './dots/results/<result_dir>' ; where in dfg_name '*_B<some_value>_*', some_value == blk):
            run cmd: mapper with <map_dfg_path=<result_dir>/dfg_name>, <map_dfg_name=dfg_name>
/end_pylike
